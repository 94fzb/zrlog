...
for url in "${urls[@]}"; do
  ...
  localPath="conf/plugins/installed-plugins/$filename"
  wget --user-agent="${UA}" -O "${localPath}" "${mirrorWebSite}/plugin/$url?v=$(date +%s)"
done
pluginLocalPath="conf/plugins/plugin-core-${arch}.bin"
...
#cd ..
ln -sf zrlog bootstrap
./mvnw -Dproject.build.outputTimestamp=2013-01-01T00:00:00Z -Dmaven.test.skip=false -DskipTests=false -Ppackage-faas-zip assembly:single -f "package/pom.xml"

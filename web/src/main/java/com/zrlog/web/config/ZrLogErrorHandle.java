...
public class ZrLogErrorHandle implements HttpErrorHandle {
    ...
    public ZrLogErrorHandle(int httpStatueCode, ZrLogConfig zrLogConfig) {
    ...
    public void doHandle(HttpRequest request, HttpResponse response, Throwable e) {
        ...
        if (zrLogConfig.isStaticPluginRequest(request)) {
            return;
        }
        ...
        if (request.getUri().startsWith(AdminConstants.ADMIN_URI_BASE_PATH)) {
            if (e instanceof NotFindResourceException || e instanceof NotFindDbEntryException) {
                response.redirect(AdminConstants.ADMIN_URI_BASE_PATH + "/404?queryString=" + request.getQueryStr() + "&uriPath=" + request.getUri() + "&message=" + e.getMessage());
            }
            response.redirect(AdminConstants.ADMIN_URI_BASE_PATH + "/500?message=" + e.getMessage());
            return;
        }
        if (Constants.debugLoggerPrintAble()) {

...
<aside>
	...
	<div class="widget search">
		<form id="searchform" action="${searchUrl}" method="post">
			<p class="search_input"><input type="text" size="15" name="key"  placeholder="${_res.searchTip}"  value="${key}" class="inputtext"><input type="submit" class="btn" value="${_res.search}"></p>
		</form>
	</div>
	<c:choose>
	...
	<div class="taglist" id="tags">
		 <c:forEach items="${init.tags}" var="tag">
			<a href="${tag.url}" class="size${tag.count%6}" title="${tag.text}上共有(${tag.count})文章">${tag.text}</a>
		 </c:forEach>
	</div>
	</div>
							</c:when>

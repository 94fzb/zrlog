...
public class PluginCorePluginImpl extends BaseLockObject implements PluginCorePlugin {
    ...
    public PluginCorePluginImpl(File dbPropertiesPath) {
        ...
        this.pluginFolder = new File(EnvKit.isLambda() ? ZrLogUtil.getLambdaRoot() + "/conf/plugins" : PathUtil.getConfPath() + "/plugins");
    }
    ...
    private String getPluginFileName() {
        if (Constants.runMode.isNative()) {
            if (NativeUtils.getRealFileArch().contains("Window")) {
                return "plugin-core-" + NativeUtils.getRealFileArch() + ".exe";
            }
            return "plugin-core-" + NativeUtils.getRealFileArch() + ".bin";
        }
    ...
    @Override
    public void refreshCache(long cacheVersion) {
    ...
    private static void waitToStarted(String pluginServerBaseUrl, String token, int retryCount) {
        if (retryCount < 0) {
            LOGGER.log(Level.SEVERE, "refresh plugin cache error, timeout");
            return;
        }
        int seek = 1000;
        try {
            ...
            if (Objects.isNull(statusResponse.getStatus()) || !Objects.equals(statusResponse.getStatus(), PluginCoreStatus.STARTED)) {
                int rCount = retryCount - 1;
                Thread.sleep(seek);

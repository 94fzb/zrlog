...
public class PluginCoreProcessImpl implements PluginCoreProcess {
    ...
    private Process startPluginCore(final File pluginCoreFile, final String dbProperties, final String pluginJvmArgs,
        ...
        args.add(getInstalledPluginFolder(pluginCoreFile));
    ...
    private String getInstalledPluginFolder(File pluginCoreFile) {
        String installFolder = pluginCoreFile.getParent() + "/installed-plugins";
        if (EnvKit.isLambda()) {
            File workFolder = new File("/tmp/conf/plugins/installed-plugins");
            workFolder.mkdirs();
            FileUtils.moveOrCopyFolder(installFolder, workFolder.getParent(), false);
            return workFolder.toString();
        }

...
public class DbUpgradeService {
    ...
    private static final String ZRLOG_SQL_VERSION_KEY = "zrlogSqlVersion";
    ...
    private Long getCurrentSqlVersion() {
        try {
            return (long) Double.parseDouble(webSite.getStringValueByName(ZRLOG_SQL_VERSION_KEY));
        } catch (Exception e) {
    ...
    public void tryDoUpgrade() {
        try {
            Long currentVersion = getCurrentSqlVersion();
            ...
            webSite.updateByKV(ZRLOG_SQL_VERSION_KEY, SQL_VERSION + "");
        } catch (Exception e) {
            LOGGER.log(Level.SEVERE, "", e);
        }
    }

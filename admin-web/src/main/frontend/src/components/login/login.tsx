    ...
    axios
      ...
      .then(({ data }) => {
        if (data.error) {
          ...
          if (
            query.get("redirectFrom") !== null &&
            query.get("redirectFrom") !== ""
          ) {
            const jumpTo = decodeURIComponent(query.get("redirectFrom") + "");
            //need reload page, for iframe (maybe)
            if (
              jumpTo.startsWith(
                new URL(document.baseURI).pathname + "admin/plugins/download"
              )
            ) {
  ...
  return (
    <Layout>
      ...
      <Footer style={{ textAlign: "center" }}>
        {getRes().copyrightCurrentYear} {getRes().websiteTitle}. All Rights
        Reserved.
      </Footer>
    </Layout>
  );

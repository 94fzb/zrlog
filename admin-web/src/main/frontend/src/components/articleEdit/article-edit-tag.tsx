...
const ArticleEditTag: FunctionComponent<ArticleEditTagProps> = ({ allTags, keywords }) => {
    ...
    const tagForMap = (tag: string) => {
        const tagElem = (
            <Tag icon={<TagOutlined />} onClick={(e) => allTagsOnClick(e)} closable={false} color={getColorPrimary()}>
                {tag}
            </Tag>
        );
        return (
    ...
    return (
        <>
            <Input id="keywords" value={state.keywords} style={{ display: "none" }} />
            ...
            {!inputVisible && (
                <>
                    ...
                    <div style={{ maxHeight: "120px", overflowY: "scroll" }}>{allTagChild}</div>
                </>
            )}
        </>
    );

...
public class AdminResourceImpl implements AdminResource {
    ...
    public AdminResourceImpl(CacheService<BaseDataInitVO> cacheService, String contextPath) {
    ...
    private List<String> buildRealPageUrls(String e, String adminResourceUrl, HttpRequest request) {
        ...
        if (e.endsWith("/?pwa=true") || e.endsWith("/")) {
            ...
            sb.append(Constants.zrLogConfig.isStaticPluginRequest(request) ? ".html?" : "?");
        }
    ...
    public ByteArrayInputStream renderServiceWorker(HttpRequest request) {
        ...
        return new ByteArrayInputStream(IOUtil.getStringInputStream(AdminResourceImpl.class.getResourceAsStream(Constants.ADMIN_SERVICE_WORKER_JS)).replace("const urlsToCache = []", newUrls).getBytes());
    }

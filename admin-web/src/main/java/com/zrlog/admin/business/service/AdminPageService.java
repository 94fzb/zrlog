...
public class AdminPageService {
        ...
        if (!manifest.isEmpty()) {
            ...
            if (Objects.nonNull(manifestElement)) {
                ...
                if (request.getUri().startsWith(Constants.ADMIN_LOGIN_URI_PATH)) {
                    ...
                    String newUrl;
                    if (StringUtils.isNotEmpty(adminStaticResourceHostByWebSite) && !Constants.zrLogConfig.isStaticPluginRequest(request)) {
    ...
    public ServerSideDataResponse<Object> serverSide(String uri, HttpRequest request, HttpResponse response) throws Throwable {
        Map<String, Object> resourceInfo = new AdminResourceService().adminResourceInfo(request);
        if (Objects.nonNull(AdminTokenThreadLocal.getUser())) {
            ...
            try {
                ...
                if (Objects.nonNull(result)) {
                    ...
                    return new ServerSideDataResponse<>(basicInfoResponse, resourceInfo, result.getData(), AdminTokenThreadLocal.getUser().getSessionId(), AdminConstants.getAdminDocumentTitleByUri(request.getUri()));
                } else {

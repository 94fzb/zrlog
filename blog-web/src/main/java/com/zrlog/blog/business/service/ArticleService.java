...
public class ArticleService {
    ...
    private List<Map<String, Object>> getTags(Map<String, Object> log, HttpRequest request) {
        ...
        for (String tag : keywords.split(",")) {
            ...
            map.put("name", tag);
            map.put("url", WebTools.buildEncodedUrl(request, "tag/" + tag + StaticSitePlugin.getSuffix(request)));
            tags.add(map);
        }
        return tags;
    }
    ...
    public Map<String, Object> detail(Object idOrAlias, HttpRequest request) throws SQLException {
        ...
        try {
            ...
            futures.add(CompletableFuture.runAsync(() -> {
                if (ResultValueConvertUtils.toBoolean(log.get("canComment")) && Constants.isAllowComment()) {
                    try {
                    ...
                    log.put("comments", new ArrayList<>());
                }
            }));
